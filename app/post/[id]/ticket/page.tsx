...
export default function TicketPage() {
  ...
  const pillItems = useMemo(() => {
    ...
    // 4. Other add_details (starting point, dress code, links) – show value or field name. No location tag.
    addDetails.forEach((d: any) => {
      if (!d || items.length >= 4) return;
      const t = (d.detail_type || '').toLowerCase();
      if (t === 'distance' || t === 'f&b' || t === 'location' || t === 'venue') return;
  ...
  const InnerTicket = ({ isDesktopLayout = false }: { isDesktopLayout?: boolean }) => {
    ...
    <div className={isDesktopLayout ? 'flex min-h-full flex-col' : 'flex h-full min-h-0 flex-col overflow-hidden'}>
      ...
      <p className="shrink-0 text-center text-[11px] font-medium text-[#1C1C1E] px-4 pb-2 whitespace-nowrap truncate">
        Your pass will be sent to you via Whatsapp shortly
      </p>

      {/* Ticket + View Event: fit in viewport without scroll */}
      <div className="relative flex min-h-0 flex-1 flex-col items-center overflow-hidden">
        <div
          ref={ticketContentRef}
          className="flex min-h-0 w-full max-w-[420px] flex-1 flex-col items-center px-4 pb-2 pt-0"
          style={{
            background: 'linear-gradient(180deg, #8B7AB8 0%, #C9A0B8 35%, #F5E6E8 70%, #FFFFFF 100%)',
          }}
        >
          <div className="relative flex min-h-0 w-full max-w-[380px] flex-1 flex-col">
            {/* Ticket card - constrained height so no scroll */}
            <div className="relative z-[2] flex min-h-0 flex-1 flex-col">
              <div className="mb-0 flex min-h-0 flex-1 overflow-hidden rounded-[20px] bg-white" style={{ boxShadow: 'rgba(0, 0, 0, 0.35) 0px 5px 15px' }}>
                <div className="relative flex min-h-0 w-full flex-1 flex-col overflow-hidden rounded-t-[20px]">
                  {/* Image: cap height so card fits viewport */}
                  {mainImage ? (
                    <div className="relative h-[min(26dvh,140px)] min-h-[80px] w-full shrink-0 overflow-hidden">
                      <img
                    ...
                    <div className="flex h-[min(26dvh,140px)] min-h-[80px] w-full shrink-0 items-center justify-center bg-[#94A3B8]">
                  ...
                  {mainImage && (
                    <>
                      <div
                        className="absolute bottom-0 left-0 right-0 z-10 h-[60%] min-h-[56px] pointer-events-none"
                        aria-hidden
                        style={{
                          background: 'linear-gradient(180deg, transparent 0%, rgba(0,0,0,0.4) 40%, rgba(0,0,0,0.75) 100%)',
                          backdropFilter: 'blur(4px)',
                        }}
                      />
                      <div className="absolute inset-x-0 bottom-0 pt-2 pb-3 px-4 z-20 pointer-events-none">
                        <h2 className="text-lg font-extrabold leading-tight text-white">{plan.title ?? 'Event'}</h2>
                        <div className="mt-1 flex items-center gap-2 text-xs font-semibold text-white/95">
                        ...
                        {plan.location_text && (
                          <p className="mt-0.5 flex items-center gap-1 overflow-hidden text-ellipsis whitespace-nowrap text-[11px] text-white/85">
                ...
                {/* Info section - pills + QR, compact */}
                <div
                  className="relative z-[1] flex shrink-0 gap-3 rounded-b-[20px] rounded-t-[16px] bg-white px-4 py-3"
                  style={{ marginTop: -overlapAmount, paddingTop: overlapAmount + 10, boxShadow: 'rgba(0, 0, 0, 0.35) 0px 5px 15px' }}
                >
                  <div className="flex min-w-0 flex-1 flex-col items-start justify-center gap-1.5">
                    {pillItems.map((item, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="inline-flex h-[28px] min-w-0 max-w-full items-center gap-1.5 rounded-[14px] border border-[#E5E7EB]/80 bg-[#F2F2F7] pl-2 pr-2.5 shadow-sm"
                        >
                          <span className="flex shrink-0 items-center justify-center text-[#1C1C1E]">
                            <Icon className="h-3.5 w-3.5" />
                          </span>
                          <span className="min-w-0 flex-1 truncate text-[11px] font-medium leading-none text-[#1C1C1E]">
                            {item.label}
                          </span>
                        </div>
                      );
                  ...
                  <div className="flex min-w-[80px] shrink-0 flex-col items-center justify-center pb-0">
                    <div className="rounded-lg border border-[#E5E7EB] bg-white p-1.5">
                      {qrDataUrl ? (
                        <img src={qrDataUrl} width={80} height={80} alt="" className="block size-[80px]" />
                      ) : ticket?.qr_code_hash ? (
                        <QRCodeSVG value={ticket.qr_code_hash} size={80} level="M" />
                      ) : (
                        <div className="flex h-[80px] w-[80px] items-center justify-center rounded bg-[#F3F4F6] text-[#8E8E93]">
                    ...
                    <p className="mt-1 text-center text-sm font-bold leading-tight text-[#1C1C1E]">{passName}</p>
                    <p className="mt-0.5 max-w-full truncate px-0.5 text-center text-[11px] font-medium tracking-wide text-[#6B7280]">
        ...
        {/* View Event button - same screen, no scroll */}
        <Link
          href={`/post/${planId}`}
          className="mt-2 mb-4 w-full max-w-[380px] shrink-0 rounded-[22px] bg-[#1C1C1E] py-3.5 text-center text-sm font-bold text-white no-underline shadow-lg hover:bg-[#2d2d2d]"
        >
          View Event
        </Link>
      </div>

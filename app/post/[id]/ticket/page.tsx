...
export default function TicketPage() {
  ...
  const pillItems = useMemo(() => {
    ...
    // 2. Distance – show value (e.g. 5km) or "Distance: value" or fallback to location_text
    const distanceDetail = detailBy('distance');
    const distance = getDetailLabel(distanceDetail, plan.location_text || '');
    ...
    // 4. Other add_details (starting point, dress code, links) – show value or field name
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-y-auto">
        ...
        >
          <div className={`relative w-full max-w-[420px] ${isDesktopLayout ? 'flex-shrink-0' : 'flex-shrink-0'}`}>
            ...
            {/* Main ticket card - image on top, text overlay on top of image (higher z-index) */}
            <div className="mb-0 overflow-hidden rounded-[24px] bg-white" style={{ boxShadow: 'rgba(0, 0, 0, 0.35) 0px 5px 15px' }}>
              <div className="relative w-full overflow-hidden rounded-t-[24px]">
                ...
                {mainImage && (
                  <>
                    {/* Blur + gradient: upper part of strip lighter, gets stronger toward bottom (180deg = top to bottom) */}
                    <div
                      className="absolute bottom-0 left-0 right-0 z-10 h-[25%] min-h-[72px] pointer-events-none"
                      aria-hidden
                      style={{
